# Amiberry Android - robust short FetchContent/object paths for Windows hosts
#
# Loaded at the end of Amiberry's top-level project() through
# CMAKE_PROJECT_amiberry_INCLUDE, before cmake/Dependencies.cmake invokes
# FetchContent.
#
# Android Studio/AGP keeps the main CMake build tree deeply nested below
# android/app/.cxx. SDL3_image then creates additional nested codec build trees
# (libwebp, libjxl, libtiff, ...). On Windows, Ninja can fail with the misleading
# message "mkdir(...): No such file or directory" when an object path becomes
# too long.
#
# v7 solves both layers:
#   1. FetchContent is relocated to a very short directory at the root of the
#      SAME drive as the Amiberry source tree.
#   2. CMAKE_OBJECT_PATH_MAX is reduced so CMake hashes long object paths sooner.
#
# ABI and build type remain separated. A short hash of CMAKE_SOURCE_DIR prevents
# collisions if multiple Amiberry source trees exist on the same drive.
...
if(CMAKE_SYSTEM_NAME STREQUAL "Android")
    ...
    # Apply core.longpaths only to Git processes spawned by this CMake configure.
    # This does NOT modify the user's global/system Git configuration.
    if(CMAKE_HOST_WIN32)
    ...
    # Obtain the Windows drive root (for example "E:") from the source path.
    ...
    if(CMAKE_HOST_WIN32 AND NOT _AMIBERRY_ROOT_NAME STREQUAL "")
        ...
        # Non-Windows fallback; normally this file is only relevant on Windows.
        set(_AMIBERRY_FETCHCONTENT_ROOT
    ...
    # CMake normally hashes object paths only after reaching its platform limit.
    # Ninja/Win32 can fail earlier in deeply nested third-party projects.
    # 180 is safely above CMake's documented minimum of 128 while forcing
    # earlier hashing for libwebp/libjxl object trees.
    set(
